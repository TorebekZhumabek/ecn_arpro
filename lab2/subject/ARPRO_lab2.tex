...
\begin{itemize}
 ...
 \item One for a 2D motion: \texttt{Twist} with attributes \texttt{vx, vy, wz} 
...
\medskip\item[\textbf{\underline{Q3}}] In this question we will build the \texttt{update} function of the range sensor.
...
\medskip\item[\textbf{\underline{Q4}}] In the \texttt{Robot::moveWithSensor} method we currently do not take into account the sensor measurement. 
...
\SetKwComment{Comment}{(}{)}
% \SetAlgoLined
\KwData {Current twist $T$, current range $s$, gain $g$, minimum range $sm$}
\KwResult {Modified twist $T$}
\If{$T.vx > g*(s-sm)$}{$T.vx \gets g*(s-sm)$\;}
\Return $T$
\caption{Modify the given twist to avoid collisions}
\label{algo:range}
\end{algorithm}

\medskip\item[\textbf{\underline{Q5}}] Modify the \texttt{Robot::moveWithSensor} method so that we also check the current robot twist. The robot should then stay inside the walls.
\medskip\item[\textbf{\underline{Q6}}] Add some other range sensors to the robot to check if everything is going fine.
\end{itemize}
\end{document}
