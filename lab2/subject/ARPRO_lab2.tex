...
The \texttt{Robot} class already has a attribute called \texttt{sensors\_} which is a vector of \texttt{SensorWithPose}. A \texttt{SensorWithPose} is a simple structure that stores:
\begin{itemize}
 \item a \texttt{Pose} called pose
 \item a \texttt{Sensor*} called sensor. As the \texttt{Sensor} class is abtract it is forbidden to use it by itself, but pointers are still possible.
\end{itemize}

For example to print the pose corresponding to all \texttt{SensorWithPose}, we can write this inside a \texttt{Robot} method:
...
\end{lstlisting}
\end{center}
Similarly, to update all sensors with a given pose \texttt{p}, we can write the following:
\begin{center}\cppstyle
\begin{lstlisting}
    Pose p;
    for(auto & sensor: sensors_)
	sensor.sensor->update(p);
\end{lstlisting}
