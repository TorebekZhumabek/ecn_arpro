...
In this section, we program the computer to solve a given problem. It may be defined in a file or be randomly generated.
The use of the STL and of the \texttt{math.h} functions will be of good use.

\subsection{Tower of Hanoi}

This one is a classical mathematical puzzle.
A given number disks of different sizes are placed on three rods.
The only constraint is that a bigger disk cannot be above a smaller one. 

The goal is to move the disks one at a time in order to have them all on a single rod.
...
\begin{itemize}
 ...
 Then $Y$ is computed as $Y=aX+b$ and you have your set of X and Y coordinates. You can also add some random numbers to make it more difficult to the algorithm.
